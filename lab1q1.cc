...
main (int argc, char *argv[])
{
  ...
  ApplicationContainer serverApps = echoServer.Install (nodes.Get (2));//Here you give the final destination
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (10.0));

  UdpEchoClientHelper echoClient (interfaces1.GetAddress (1), 9);//we want the echo from the second interface's second node
