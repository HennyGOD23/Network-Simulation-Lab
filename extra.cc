...
main (int argc, char *argv[])
{
  ...
  devices3 = pointToPoint.Install (nodes.Get(2),nodes.Get(4));
  ...
  ApplicationContainer serverApps = echoServer.Install (nodes.Get (4));//Here you give the final destination
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (10.0));
  
  ApplicationContainer serverApps1 = echoServer1.Install (nodes.Get (3));//Here you give the final destination
  serverApps1.Start (Seconds (1.0));
  serverApps1.Stop (Seconds (10.0));


  UdpEchoClientHelper echoClient (interfaces3.GetAddress (1), 9);//we want the echo from the second interface's second node
  echoClient.SetAttribute ("MaxPackets", UintegerValue (10));
  echoClient.SetAttribute ("Interval", TimeValue (Seconds (0.5)));
  echoClient.SetAttribute ("PacketSize", UintegerValue (1024));
  
  UdpEchoClientHelper echoClient1 (interfaces2.GetAddress (1), 10);//we want the echo from the second interface's second node
